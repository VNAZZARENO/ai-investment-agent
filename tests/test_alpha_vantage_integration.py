...
class TestAlphaVantageIntegration:
    ...
    async def test_av_fallback_fetches_data_when_available(self, mock_get_av_fetcher):
        ...
        mock_av.get_financial_metrics = AsyncMock(return_value={
            'symbol': '0005.HK',
            'trailingPE': 8.5,
            'priceToBook': 0.9,
            'marketCap': 150e9,
    ...
    async def test_av_included_in_parallel_fetch(self, mock_get_av_fetcher):
        ...
        # Mock other sources
        with patch.object(fetcher, '_fetch_yfinance_enhanced', new=AsyncMock(return_value=None)):
            with patch.object(fetcher, '_fetch_yahooquery_fallback', return_value=None):
                with patch.object(fetcher, '_fetch_fmp_fallback', new=AsyncMock(return_value=None)):
                    with patch.object(fetcher, '_fetch_eodhd_fallback', new=AsyncMock(return_value=None)):
...
class TestAlphaVantageStatsTracking:
    ...
    @patch('src.data.fetcher.ALPHA_VANTAGE_AVAILABLE', True)
    @patch('src.data.fetcher.get_av_fetcher')
    async def test_alpha_vantage_stats_incremented_on_success(self, mock_get_av_fetcher):
    ...
    @patch('src.data.fetcher.ALPHA_VANTAGE_AVAILABLE', True)
    @patch('src.data.fetcher.get_av_fetcher')
    async def test_alpha_vantage_stats_not_incremented_on_failure(self, mock_get_av_fetcher):
